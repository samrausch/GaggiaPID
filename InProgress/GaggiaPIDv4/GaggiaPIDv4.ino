...
#ifndef STASSID
#define STASSID "hotcockmachine"
#define STAPSK "starbucks"
#endif
...
const int line2 = 16;
const int line3 = 32;
const int upPin = 8; // Pi Pico pin
const int modePin = 7; // Pi Pico pin
const int downPin = 6; // Pi Pico pin
...
void setup() {
  ...
  tc.enable(true);
  pinMode(modePin, INPUT_PULLUP);  modeButton.attach(modePin);  modeButton.interval(intervalMs);  modeButton.setPressedState(LOW);
  pinMode(upPin, INPUT_PULLUP);  upButton.attach(modePin);  upButton.interval(intervalMs);  upButton.setPressedState(LOW);
  pinMode(downPin, INPUT_PULLUP);  downButton.attach(modePin);  downButton.interval(intervalMs);  downButton.setPressedState(LOW);
